...
8장 산출물(registry에서 추출한 피처 정의 + 7장 일별 확정 집계)을 입력으로,
...
#   complaint_count : 9장 훈련 쌍의 입력, 10장 champion 서빙 입력, 14장 통합 MVP 소비
#   day_unmapped_rate: complaint_model_v1 서비스에 등록된 품질 피처(9장 훈련 서비스)
FEATURE_CONSUMERS = {
    "complaint_count": ["complaint_model_v1 훈련(9장)", "champion 서빙(10장)", "통합 MVP(14장)"],
...
def load_inputs(input_dir: Path) -> tuple[dict, list[dict], list[dict]]:
    """8장 정의 JSON + 7장 일별 집계 3일치를 읽어 피처 정의와 원천 9행을 만든다."""
    definitions = json.loads((input_dir / "ch8_feature_definitions.json").read_text("utf-8"))

    days = sorted(p.name for p in (input_dir / "ch7_daily").iterdir() if p.is_dir())
    feature_rows: list[dict] = []
    quality_by_day: list[dict] = []
    for day in days:
        daily = json.loads((input_dir / "ch7_daily" / day / "daily_summary.json").read_text("utf-8"))
        quality = json.loads((input_dir / "ch7_daily" / day / "quality_check.json").read_text("utf-8"))
...
def check_quality(feature_rows: list[dict]) -> dict:
    """피처 단위 품질 규칙 3종을 8장 실데이터 9행에 적용(11장 분포 감시와 다른 층)."""
    n = len(feature_rows)
...
def build_audit_checklist(catalog: list[dict], access: dict,
    ...
    """access_log·카탈로그·품질·변경 이력에서 감사 점검표를 자동 생성한다(14장 선례)."""
    log = access["log"]
...
def build_governance(input_dir: Path, output_dir: Path) -> dict:
    ...
    report = {
        ...
        "inputs": {
            "feature_definition_source": "ch8_feature_definitions.json(8장 registry 추출)",
    ...
    (output_dir / "ch13_governance_report.json").write_text(
        json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n", "utf-8")
    (output_dir / "ch13_audit_checklist.json").write_text(
