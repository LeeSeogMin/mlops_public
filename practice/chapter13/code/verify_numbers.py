#!/usr/bin/env python3
"""본문(docs/chapter13.md) 인용 수치 ↔ 증거 JSON 전수 대조.

증거 JSON(ch13_governance_report.json)에서 기대값을 읽어,
본문이 인용하는 핵심 수치가 (1) 증거와 일치하고 (2) 본문에 실제로 등장하는지 검사한다.
모두 통과하면 ALL_PASS를 출력한다.

실행: cd practice/chapter13 && venv/bin/python code/verify_numbers.py
"""
...
REPORT = BASE / "data" / "output" / "ch13_governance_report.json"
DOCS = BASE.parent.parent / "docs" / "chapter13.md"


def main() -> int:
    if not REPORT.exists():
        print(f"증거 없음: {REPORT} — run_chapter13.py 먼저 실행", file=sys.stderr)
        return 2
