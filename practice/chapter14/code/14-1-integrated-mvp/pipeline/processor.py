#!/usr/bin/env python3
"""14장 통합 MVP: processor — 소비·정제·멱등 반영·일 마감 집계·예측 호출.

앞 장 부품의 재조립이다(새 규칙 없음):
- 정제 4갈래(공백 보정·약칭 보정·표준코드 매핑·미기재/미매핑 분리 보고): 7장 규약
- 멱등 반영(complaint_id UPSERT — 재전송·중복 접수를 업무 상태 변경 없이 흡수): 5장 규약
- 일 마감(_SUCCESS 마커는 마지막에, 마커 있으면 재마감 안 함): 7장 규약
...
def dump_json(path: Path, obj) -> None:
    """결정적 직렬화(키 정렬·고정 포맷) — 재실행 산출물 비교의 전제(7장 규약)."""
...
def ingest_complaint(db: sqlite3.Connection, rec: dict, mapping: dict[str, str]) -> None:
    ...
    # 5장 UPSERT: 같은 complaint_id의 재수신(수송 재전송이든 중복 접수든)은
...
def close_day(db: sqlite3.Connection, date: str, mapping: dict[str, str]) -> None:
    ...
    # 마커는 마지막에 쓴다(7장 규약) — 마커가 있으면 이 날의 집계는 확정본이다
...
def main() -> None:
    ...
    # 이유는 멱등 설계(UPSERT·마커·키 쓰기)다 — 4·5장 규약의 소비자판.
    consumer = KafkaConsumer(TOPIC, bootstrap_servers=BOOTSTRAP,
                             group_id="ch14-processor",
