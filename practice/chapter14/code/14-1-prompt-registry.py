...
결정적으로 구현한다. LLM의 실제 생성(비결정)은 14-2 스냅샷으로 분리한다.
...
원칙
  ...
  - 실제 생성 호출은 이 파일에 없다(비결정) — 14-2-llm-snapshot.py가 담당.
...
def approve_prompt_change(conn: sqlite3.Connection, registry: dict,
    ...
    """프롬프트 v1→v2 변경을 human-in-the-loop 승인 기록으로 적재(12주차 change_log 선례)."""
    conn.execute("""
...
def build_risk_checklist(corpus: dict, registry: dict, queries: dict,
                         approval: dict) -> dict:
    """로그·레지스트리·코퍼스에서 LLM 서비스 위험 체크리스트를 자동 생성(12주차 선례)."""
    log = queries["log"]
...
def build_report(input_dir: Path, output_dir: Path) -> dict:
    ...
    report = {
        ...
        "generation": {
            ...
            "note": "실제 생성은 14-2-llm-snapshot.py의 비결정 스냅샷(재현 게이트 밖)",
        },
    }
    (output_dir / "ch14_llmops_report.json").write_text(
        json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n", "utf-8")
    (output_dir / "ch14_audit_checklist.json").write_text(
...
def _write_policy_doc(output_dir: Path, registry: dict, approval: dict) -> None:
    ...
    (output_dir / "ch14_prompt_governance_policy.md").write_text("\n".join(lines), "utf-8")
