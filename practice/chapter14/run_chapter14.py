#!/usr/bin/env python3
"""14장 실습 14.1 오케스트레이션: 통합 MVP의 빌드·기동·시나리오·드릴·검수.
...
실행: python run_chapter14.py            (chapter14 venv, Docker 데몬 필요)
      python run_chapter14.py --keep-up  (종료 후 스택 유지 — 수동 관찰용)
콘솔 전체가 data/output/ch14_run_log.txt에 남는다(재실행 산출물 — 커밋 제외).
"""
...
MVP_DIR = BASE_DIR / "code" / "14-1-integrated-mvp"
...
LOG_PATH = OUT_DIR / "ch14_run_log.txt"
_log_fh = None
...
def main() -> int:
    ...
    if boot.returncode != 0 or "CH14_BOOTSTRAP_OK" not in boot.stdout:
    ...
    for img in ("ch14-model-api", "ch14-pipeline"):
    ...
    try:
        ...
        (OUT_DIR / "ch14_smoke.json").write_text(
        ...
        say(f"증거 파일: {OUT_DIR / 'ch14_integration_report.json'}")
        say("CH14_RUN_PASS")
        return 0
    finally:
        if keep_up:
            say("(--keep-up: 스택 유지 — 수동 정리: docker compose -p ch14-mvp down -v)")
        else:
            compose("down", "-v", "--remove-orphans", check=False)
        _log_fh.close()
