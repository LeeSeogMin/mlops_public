#!/usr/bin/env python3
"""12주차 피처 거버넌스 실습 오케스트레이션: 피처 거버넌스 보고서 생성 + PASS 게이트.

  1. 7주차 정의 + 6주차 일별 집계 스냅숏을 읽어 카탈로그·접근로그·품질·변경관리·감사표 생성
  2. 증거 JSON(ch12_governance_report.json, ch12_governance_checklist.json) 저장
...
CODE = BASE_DIR / "code" / "12-1-feature-governance.py"
...
def main() -> int:
    if not (INPUT / "ch7_feature_definitions.json").exists():
    ...
    print("CH12_GOVERNANCE_RUN_PASS" if not failures else f"CH12_GOVERNANCE_RUN_FAIL({'; '.join(failures)})")
