...
- 단독 실행: python code/7-1-daily-report-dag.py  (2026-07-01 하루치 실행)
  전체 시나리오: python run_chapter7.py
"""
...
def clean_and_map(ds: str, **_) -> dict:
    """중복 제거(5장) → 표기 정규화 → 표준 코드 매핑. 총계 보존을 기록한다."""
    path = INPUT_DIR / "complaints" / f"{ds}.jsonl"
    ...
    # (1) 중복 접수 제거: 같은 complaint_id의 재전송은 먼저 온 것만 남긴다(5장 원칙).
    seen: set[str] = set()
