#!/usr/bin/env python3
"""8장 실습 8.1: Feast 기반 최소 피처 저장소 구성.

7장 배치가 확정한 일별 민원 집계(스냅숏 동봉)를 피처 원천으로 삼아,
...
- 원천 데이터는 7장 실측 산출물의 스냅숏이다(민원 자체는 4~7장과 동일한
  시뮬레이션 입력). 관찰 대상은 실제 Feast 엔진의 조인·적재·조회 동작이며,
  본문 인용 수치는 전부 실제 실행 산출물에서 나온다.
- 실행: python code/8-1-feature-store-minimal.py  (또는 python run_chapter8.py)
"""
...
# ── 1단계: 피처 원천 만들기 — 7장 확정 집계 → parquet ─────────────
def build_source_parquet() -> Path:
    ...
    rows = []
    for day in DAYS:
        summary = json.loads((INPUT_DIR / "ch7_daily" / day / "daily_summary.json").read_text(encoding="utf-8"))
        check = json.loads((INPUT_DIR / "ch7_daily" / day / "quality_check.json").read_text(encoding="utf-8"))
...
""",
    ...
    district = Entity(
        ...
        description="법정동 시군구 코드(행정표준코드) — 7장 정제의 표준 키",
    )
    ...
    daily_fv = FeatureView(
        ...
        schema=[
            ...
            Field(name="day_unmapped_rate", dtype=Float64,
                  description="전일 정제 품질 — 매핑 실패율(7장 quality_check)"),
        ],
...
def main() -> int:
    ...
    dump_json(OUTPUT_DIR / "ch8_feature_report.json", report)
    dump_json(OUTPUT_DIR / "ch8_feature_definitions.json", defs)
    dump_json(OUTPUT_DIR / "ch8_consistency_report.json", consistency)

    # 종합 판정: point-in-time 기대값(7장 실측에서 손으로 도출)과 일치하는가
    expected = {
    ...
    print("CH8_RUN_" + ("PASS" if ok else "FAIL"))
