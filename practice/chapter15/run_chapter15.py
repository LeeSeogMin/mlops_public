#!/usr/bin/env python3
"""15장 실습 15.1 오케스트레이션: LLMOps 결정적 코어 실행 + PASS 게이트.

  1. 코퍼스(개인정보 보호법 조문) 로드 → 프롬프트 레지스트리·RAG·위험 통제·승인·체크리스트 생성
  2. 결정적 증거 JSON(ch15_llmops_report.json, ch15_audit_checklist.json) 저장
  3. PASS 게이트: 실습이 시연하려는 불변식을 코드로 검증

실제 LLM 생성(비결정)은 이 오케스트레이터에 포함하지 않는다 — 15-2-llm-snapshot.py가
...
CODE = BASE_DIR / "code" / "15-1-prompt-registry.py"
...
def main() -> int:
    ...
    print("CH15_RUN_PASS" if not failures else f"CH15_RUN_FAIL({'; '.join(failures)})")
