...
결정적 코어(15-1)가 "어느 조문이 근거인가"와 "생성해도 되는가"를 정한다.
...
원칙
  ...
  - 이 파일은 run_chapter15.py(PASS 게이트)에 포함되지 않는다(키 없이 오프라인 통과 보장).
  - API 키는 환경변수에서만 읽는다(평문 커밋 금지). 키·환경변수 이름을 출력에 남기지 않는다.
  - 한 제공자가 실패해도(키·요금제·장애) 정직히 기록하고, 다른 제공자 결과는 남긴다(우회 금지).

실행: cd practice/chapter15 && \
      CLOVA_STUDIO_API_KEY=... OPENAI_API_KEY=... venv/bin/python code/15-2-llm-snapshot.py
"""
...
REPORT = OUTPUT / "ch15_llmops_report.json"
SNAPSHOT = OUTPUT / "ch15_llm_snapshot.json"
...
def main() -> int:
    if not REPORT.exists():
        print("증거 없음 — run_chapter15.py 먼저 실행", file=sys.stderr)
        return 2
    ...
    # 활성 프롬프트 원문·질의 텍스트는 결정적 코어(15-1)의 상수를 임포트해 재현한다.
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "prompt_registry", BASE / "code" / "15-1-prompt-registry.py")
    ...
    snapshot = {
        ...
        "note": ("동일한 결정적 코어(프롬프트 v2·검색·근거 조문)를 두 제공자에 그대로 넣어 실행했다. "
                 ...
                 "결정적 코어(근거·생성 허용 여부)는 ch15_llmops_report.json에 있다(12장 지연 스냅샷 선례)."),
