#!/usr/bin/env python3
"""본문(docs/chapter15.md) 핵심 수치 ↔ 결정적 증거 JSON 대조(전수 아님, 핵심 지표 한정).

결정적 증거(ch15_llmops_report.json)에서 기대값을 읽어, 본문이 인용하는 핵심 수치가
(1) 증거와 일치하고 (2) 본문에 실제로 등장하는지 검사한다(맥락 고정 문구 포함). 모두 통과 시 ALL_PASS.

범위 한계(의도적):
  - LLM 생성 스냅샷(ch15_llm_snapshot.json)의 문장은 비결정이므로 대조 대상이 아니다.
    본문의 스냅샷 서술("이번 스냅샷에서 …")은 게이트 밖이며 여기서 검증하지 않는다.
  - 이 검증기는 결정적 코어의 핵심 지표·해시·대표 점수만 확인한다(모든 문장의 전수 검증 아님).

실행: cd practice/chapter15 && venv/bin/python code/verify_numbers.py
"""
...
REPORT = BASE / "data" / "output" / "ch15_llmops_report.json"
DOCS = BASE.parent.parent / "docs" / "chapter15.md"


def main() -> int:
    if not REPORT.exists():
        print(f"증거 없음: {REPORT} — run_chapter15.py 먼저 실행", file=sys.stderr)
        return 2
