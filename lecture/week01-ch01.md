...
**HTTP 200을 성공으로 믿은 경우.** 네트워크 전송이 성공하면 HTTP 상태는 200이다. 그런데 공공 API는 200을 돌려주면서 응답 안쪽 헤더에 별도의 업무 오류 코드를 담을 수 있다(인증키 만료, 호출 한도 초과 등). HTTP 상태만 검사하는 코드는 이 오류 응답을 정상 데이터로 저장한다. 성공 판정은 두 층에서 각각 해야 한다.
...
머신러닝 수업에서는 모델을 만들고 정확도를 확인하는 데서 끝난다. 운영은 거기서 시작한다.
...
**그림 1.1** 데이터 공급부터 모델 감시까지의 연결. 위쪽은 데이터 엔지니어링, 아래쪽은 MLOps에 해당하며, 모니터링 결과가 다시 모델 개발로 돌아온다.
...
## 6. 학습-서빙 불일치 — 두 영역이 만나는 자리

데이터 엔지니어링과 MLOps가 만나는 지점은 **피처를 만들어 넘기는 자리**다. 이 자리에서 생기는 대표적인 사고가 학습-서빙 불일치(training-serving skew)다.

모델을 학습할 때는 보통 과거 데이터를 한꺼번에 처리한다. 반면 실제 예측은 지금 들어온 데이터를 즉시 처리한다. 두 경로에서 같은 피처를 서로 다른 코드로 계산하면 값이 달라진다.

- 학습에서는 결측을 평균으로 채우고, 실시간에서는 0으로 채운다.
- 학습에서는 "최근 7일"을 달력 날짜로 계산하고, 실시간에서는 현재 시각 기준 168시간으로 계산한다.

두 코드 모두 오류 없이 실행된다. 그런데 모델은 학습할 때와 다른 값을 입력으로 받는다. 성능은 떨어지는데 어느 코드도 실패하지 않으므로, 이것 역시 침묵 실패의 한 종류다.

해결 방향은 **피처의 정의와 계산을 한곳에 모으고 두 경로가 같은 정의를 쓰게 하는 것**이다. 이 목적으로 쓰는 저장소를 피처 스토어(Feature Store)라고 하며 8장에서 직접 만든다.

한 가지만 더 짚어 둔다. 학습 데이터를 만들 때는 **그 시점에 알 수 있었던 값만** 써야 한다. 예측하려는 시점보다 나중에 생긴 값을 실수로 섞으면 학습 성능이 실제보다 훨씬 좋게 나오지만, 정작 서비스에서는 그 값이 없어 성능이 급락한다. 학습 단계에서는 발견하기 어렵고 배포한 뒤에야 드러나는 오류다.
